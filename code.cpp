...
{   
	...
	{   
		...
		{   
			p->data.state=Free;   
			p->data.ID=Free;   
			if(p->prior->data.state==Free&&p->next->data.state!=Free)//与前面的空闲块相连   
			{   
				p->prior->data.size+=p->data.size;   
				p->prior->next=p->next;   
				p->next->prior=p->prior;   
			}   
			else if(p->prior->data.state!=Free&&p->next->data.state==Free)//与后面的空闲块相连   
			{   
				if(p->next->next!=NULL)
				{
			...
			else if(p->prior->data.state==Free&&p->next->data.state==Free)//前后都有空闲块时
			{
				if(p->next->next!=NULL){
